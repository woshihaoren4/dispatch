...
use crate::pb::{
    CommonResult, CreateTaskRequest, CreateTaskResponse, SearchSubTaskRequest,
    SearchSubTaskResponse, SearchTaskRequest, SearchTaskResponse, UpdateTaskRequest,
    UpdateTaskResponse,
};
...
impl TaskManagerServices for super::Server {
    async fn create_task(
        ...
        match result {
            Ok(o) => Ok(Response::new(CreateTaskResponse {
    ...
    async fn update_task(
        ...
        let t = match result {
            Ok(t) => t,
            Err(e) => return Ok(Response::new(UpdateTaskResponse { result: Some(e) })),
        };
        ...
        match req.action {
            1 => {
                ...
                if !Self::fsm(old, status) {
                    return Ok(Response::new(UpdateTaskResponse {
                        result: Server::response_err_result(
                            400,
                            format!("task status({:?}) can not to status({:?})", old, status),
                        ),
                    }));
                }
                ...
                return match result {
                    Ok(_) => Ok(Response::new(UpdateTaskResponse {
                        result: Self::response_success(),
                    })),
            ...
            3 => {
                ...
                let ast =
                    match req.update_content.unwrap() {
                        UpdateContent::SubTasks(subs) => subs,
                        _ => return Ok(Response::new(UpdateTaskResponse {
                            result: Server::response_err_result(
                                400,
                                format!(
                                    "active is append sub task,but content is not AppendSubTask"
                                ),
                            ),
                        })),
                    };
                ...
                match result {
                    ...
                    Err(e) => {
                        return Ok(Response::new(UpdateTaskResponse {
                            result: Server::response_err_result(
                                500,
                                format!("append sub task error:({})", e),
                            ),
                        }))
                    }
                }
    ...
    async fn search_task(
        ...
        query.push((
            "task_code".to_string(),
            QueryOption::Equal(Value::String(req.task_code)),
        ));
        query.push((
            "task_name".to_string(),
            QueryOption::Like(Value::String(req.name)),
        ));
        query.push((
            "type".to_string(),
            QueryOption::Equal(Value::Number(Number::from(req.r#type))),
        ));
        query.push((
            "status".to_string(),
            QueryOption::Equal(Value::Number(Number::from(req.status))),
        ));
        query.push((
            "create_time".to_string(),
            QueryOption::BetweenAnd(
                Value::Number(Number::from(req.start_time)),
                Value::Number(Number::from(req.end_time)),
            ),
        ));
        ...
        return match result {
            Ok(item) => {
                ...
                Ok(Response::new(SearchTaskResponse {
                    tasks: list,
                    total: item.1 as i32,
                    result: Self::response_success(),
                }))
            }
            Err(e) => Ok(Response::new(SearchTaskResponse {
                tasks: vec![],
                total: 0,
                result: Self::response_err_result(500, e),
            })),
...
impl super::Server {
    ...
    async fn update_task_request_check(
        &self,
        req: &Request<UpdateTaskRequest>,
    ) -> Result<entity::Task, CommonResult> {
        bad_request!(req.get_ref().task_code.is_empty(), "request code is empty");
        bad_request!(
            req.get_ref().action <= 0 || req.get_ref().action > 4,
            "unknown action"
        );
