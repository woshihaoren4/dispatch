...
impl ElectionManager {
        ...
        M: MasterAndWorker + 'static,
    {
        ...
        let ele_interval = std::time::Duration::from_secs(60);
        ...
        Self {
            ...
            master,
            election,
            master_is_worker: true, //fixme 默认应该是true
            handler,
            ele_interval,
        }
    }
    ...
    async fn election_result_handle(&self, name: String) {
        ...
        if name.eq(&self.name) && !name.eq(&master_name) {
            //当选主节点
            if !self.master_is_worker {
                self.handler.worker_stop().await;
            }
            self.handler.master_start().await;
        } else if !name.eq(&self.name) && self.name.eq(&master_name) {
            //当前节点不再是主节点
