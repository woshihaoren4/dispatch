...
impl RedisElection {
    pub fn new(cluster:String, pool:mobc::Pool<mobc_redis::RedisConnectionManager>)->Self{
        let term = 180usize;
        RedisElection{cluster,pool,term}
    }
    #[allow(dead_code)]
...
impl Election for RedisElection{
    async fn initiate_election(&self, node: String) -> anyhow::Result<String> {
        ...
        let result = redis::cmd("set").arg(format!("{}-lock",&self.cluster)).arg(&node).arg("ex").arg(5).arg("nx").query_async::<_,Option<String>>(&mut conn).await?;
        if result.is_none() { //有节点选过了，等一会重新拿一次
        ...
        return match result {
            ...
            Err(e) => {
                Err(anyhow::anyhow!("redis election error:{}",e.to_string()))
            }
        };
    }
}
